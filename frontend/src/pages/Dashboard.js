...
export default function Dashboard() {
  ...
  const subjectPieData = stats?.bySubject?.slice(0, 8).map(s => ({
  ...
  const certPieData = [
    { name: 'Muddati yaqin', value: stats?.expiringSoon?.length || 0 },
    { name: 'Yaxshi holat',  value: Math.max(0, (stats?.certCount || 0) - (stats?.expiringSoon?.length || 0)) }
  ];
    ...
    <div>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div>
            <div className="stat-num"><AnimatedNum value={stats.total} /></div>
        ...
        <div className="stat-card">
          ...
          <div>
            <div className="stat-num"><AnimatedNum value={stats.active} color="var(--success)" /></div>
        ...
        <div className="stat-card">
          ...
          <div>
            <div className="stat-num"><AnimatedNum value={stats.inactive} color="var(--danger)" /></div>
        ...
        <div className="stat-card">
          ...
          <div>
            <div className="stat-num"><AnimatedNum value={stats.certCount} color="var(--warning)" /></div>
        ...
        <div className="stat-card">
          ...
          <div>
            <div className="stat-num"><AnimatedNum value={stats.docCount} color="#7c3aed" /></div>
          ...
          <div className="card-body">
            <div style={{ display:'flex', gap:16, justifyContent:'center', marginBottom:16 }}>
              <div style={{ textAlign:'center', padding:'16px 28px', background:'#dcfce7', borderRadius:12 }}>
                ...
                <div style={{ fontSize:26, fontWeight:800, color:'#0ea571' }}>{stats.active}</div>
              ...
              <div style={{ textAlign:'center', padding:'16px 28px', background:'#fee2e2', borderRadius:12 }}>
                <div style={{ fontSize:28 }}>❌</div>
                <div style={{ fontSize:26, fontWeight:800, color:'#e53e3e' }}>{stats.inactive}</div>
                <div style={{ fontSize:12, color:'#7f1d1d', fontWeight:600 }}>Nofaol</div>
              </div>
            </div>
            <div style={{ marginTop:8 }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6, fontSize:13, fontWeight:600 }}>
                <span style={{ color:'#0ea571' }}>Faol: {stats.active} ta</span>
                <span style={{ color:'#e53e3e' }}>Nofaol: {stats.inactive} ta</span>
              </div>
              <div style={{ background:'#f1f5f9', borderRadius:8, height:12, overflow:'hidden' }}>
                <div style={{
                  background:'linear-gradient(90deg,#0ea571,#22c55e)',
                  height:'100%', borderRadius:8,
                  width:`${stats.total > 0 ? Math.round((stats.active/stats.total)*100) : 0}%`,
                  transition:'width 1.2s cubic-bezier(0.22,1,0.36,1)'
                }} />
              </div>
              <div style={{ textAlign:'center', marginTop:8, fontSize:13, color:'var(--text-muted)', fontWeight:700 }}>
                {stats.total > 0 ? Math.round((stats.active/stats.total)*100) : 0}% faol xodimlar
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="card-body">
            ...
            <div style={{ display:'flex', gap:16, justifyContent:'center', marginTop:8 }}>
              <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:13 }}>
                ...
                <span style={{ fontWeight:800 }}>{stats.expiringSoon?.length || 0}</span>
              </div>
              <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:13 }}>
                ...
                <span style={{ fontWeight:800 }}>{Math.max(0, stats.certCount - (stats.expiringSoon?.length || 0))}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── QATOR 3: OXIRGI QO'SHILGANLAR + FANLAR PROGRESS ─────────── */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
              ...
              return (
                <div key={i} style={{ marginBottom:14, animation:`fadeInUp 0.3s ${i*0.05}s both` }}>
                  ...
                  <div style={{ background:'#f1f5f9', borderRadius:6, height:8, overflow:'hidden' }}>
                    <div style={{
                      background: color, borderRadius:6, height:'100%',
                      width:`${pct}%`,
